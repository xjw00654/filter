...
def cal_ngram(
    ...
    if not isinstance(n, int):
        raise Exception('参数错误，n需要为一个整数')
    ngram_vectorizer = CountVectorizer(analyzer='char', ngram_range=(n, n))
    whitelist_list_data_rm_ext = [".".join(e.split('.')[:-1]) for e in
                                  get_whitelist_from_txt(use_v2ray=False, max_domain_length=n + 2)]
    data_ngram = ngram_vectorizer.fit_transform(whitelist_list_data_rm_ext)

    data_ct = Counter(data_ngram.indices)
